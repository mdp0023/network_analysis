...
#                                                           res_points=res_points,
#                                                           dest_points=food_points,G_capacity='capacity',G_weight='travel_time')
...
#         the flow decomposition function need to list the unique sources and sinks, the number of residents coming from each source and ending at each sink, and then relate this back to shapefiles. THIS ALSO NEEDS TO identify the households that share an intersection with a destination point - these need to be noted seperately
#         '''
...
test_function = mynet.flow_decomposition(G=G, res_points=res_points, dest_points=food_points, G_demand='demand', G_capacity='capacity', G_weight='travel_time')

print(len(test_function))
